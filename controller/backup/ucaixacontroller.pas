...
begin
  ...
  try
    try
     id_loja := DataModule1.GetIdLoja(req.Headers['Authorization']);
     jsonReq := TJSONObject(GetJSON(req.Body));
     mes := jsonReq.Find('mes').AsInteger;

     jsonRes := TCaixaModel.relatorio_entradas(id_loja, mes);
...
begin
  ...
  THorse.AddCallback(HorseJWT(DataModule1.token))
  .Post('api/v1/caixa/relatorio/entradas', HandlerRelatorioEntradas);
end;
